"""
Functin that plots the Reggiezed pion discontinuity. The main function called is
pion_disc_integrator which takes as an argument an array of s values, s being the
...
def pion_disc_integrator(SS,i):
  ...
  xmin = [ -1. ]
  xmax = [ 1. ]
  f0 = cubature( functions.pion_disc, 1, 1, xmin, xmax, args=[SS[0],alphas[i]], relerr=err  )[0]
  for j,s in enumerate(ss):
    tic = time.time()
    f = cubature( functions.pion_disc, 1, 1, xmin, xmax, args=[s,alphas[i]], relerr=err  )[0]
    fs.append(  f / f0  )
    toc = time.time()
...
plt.scatter(ss,Fs1,label=r'Reggiezed $\Delta\;F(s)$ with $\alpha=${}'.format(alphas[3]), c='k', s=5)
plt.xscale('log')
plt.yscale('log')
plt.legend()
plt.savefig('images/ReggiezedPionDiscontinuity.pdf')
plt.show()
